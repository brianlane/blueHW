...
- Always simulate first with realistic slippage model, display exact stop-loss level, and require Telegram one-tap approval before any execution
...
- Guardian-Executor has absolute veto and kill authority — no trader agent can disable or override it

MULTI-HORIZON + QUANT FORMULA LAYER + SWARM SIMULATION:
- Evaluate every opportunity across 24h / 7d / 30d / expiry horizons
...
- Scan public leaderboards, Unusual Whales smart-money flows, and correlated Kalshi/Webull activity
...
LOSSLESS CLAW + NEMOCLAW + GUARDIAN-EXECUTOR + TAILSCALE + OLLAMA PROTOCOL:
...
- Nemotron models (via NemoClaw + OpenShell sandbox) handle all critical reasoning, swarm debates, and Research Agent cycles
- Ollama quantized models handle routine 5-minute scans for cost and privacy
- Guardian-Executor runs inside the sandbox with absolute kill authority
- All SSH and external access is via Tailscale only — no public ports

SELF-MANAGEMENT + TELEGRAM FAÇADE:
- 5-minute scans during market hours with inline Approve/Reject/Simulate buttons
...
PLATFORMS:
- Stocks: Webull via Lightpanda browser automation
